...
//individuals level
  ...
  vector[Nsubjects] alpha_unch_individal_aux;
...
//declare variables and parameters
  ...
  for (subject in 1:Nsubjects) {
    ...
    alpha_unch[subject] = Phi_approx(mu_aux[2]  + sigma_aux[2]  * alpha_unch_individal_aux[subject]);
...
model {
  ...
  alpha_unch_individal_aux~ normal(0, 1);
  ...
  for (subject in 1:Nsubjects){
    ...
    real PE_unchosen;
      ...
      for (trial in 1:Ntrials_per_subject[subject]){
        ...
        //Qvalues update
        PE_chosen   = (reward[subject,trial] - Qcard[action[subject,trial]]);
        PE_unchosen = ((1-reward[subject,trial]) - Qcard[action[subject,trial]]);

        Qcard[action[subject,trial]] += alpha_ch[subject] * PE_chosen;
...
generated quantities {
  ...
  real<lower=0, upper=1>  mu_alpha_unch;
  ...
  mu_alpha_unch=Phi_approx(mu_aux[2]);
  mu_beta      =Phi_approx(mu_aux[3])*10;
  
    for (subject in 1:Nsubjects) {
    ind_alpha_ch[subject]   = Phi_approx(mu_aux[1]  + sigma_aux[1]  * alpha_ch_individal_aux[subject]);
    ind_alpha_unch[subject] = Phi_approx(mu_aux[2]  + sigma_aux[2]  * alpha_unch_individal_aux[subject]);
