...
sim.block = function(subject,parameters,cfg){ 
  ...
  #preallocation
  #set parameters
  alpha_ch = parameters['alpha_ch']
  alpha_unch = parameters['alpha_unch']
  ...
  for (block in 1:Nblocks){
    for (trial in 1:Ntrials){
      ...
      Qval[choice]   = Qval[choice] + alpha_ch*PE
      Qval[unchosen] = Qval[unchosen] + alpha_unch*(-PE)
      #normalizing the values for the range of [0,1]
      Qval[Qval>1]=1
      Qval[Qval<0]=0
    }
  }     
