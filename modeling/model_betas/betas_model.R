...
#pre-allocation
  ...
  beta_rw  = parameters['beta_rw']
  beta_unrw  = parameters['beta_unrw']
...
for (block in 1:Nblocks){
  ...
  for (trial in 1:Ntrials_perblock){
    ...
    #players choice
    if (reward_oneback==1){
    p         = exp(beta_rw*Qval[raffle]) / sum(exp(beta_rw*Qval[raffle]))
    }
    if (reward_oneback==0){
    p         = exp(beta_unrw*Qval[raffle]) / sum(exp(beta_unrw*Qval[raffle]))
    }
