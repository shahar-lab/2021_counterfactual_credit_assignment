...
#population location parameters
  ...
  mu_aux     =c(qnorm(mu_alpha),qnorm((mu_int-int_shift)/int_range),qnorm((mu_slope-slope_shift)/slope_range),qnorm(mu_beta/beta_range))
  
  #transform back to natural scale (just for practice)
  mu_alpha   =pnorm(mu_aux[1], 0,1);
  mu_int     =pnorm(mu_aux[2], 0,1)*int_range+int_shift;
  mu_slope   =pnorm(mu_aux[3], 0,1)*slope_range+slope_shift;
  mu_beta    =pnorm(mu_aux[4], 0,1)*beta_range;
  print(paste(round(mu_alpha,2),round(mu_int,2),round(mu_slope,2),round(mu_beta,2)))
  
  #scale parameter for the random effect
  sigma_aux  =c(0.75,0.75,0.75,1) 
...
#individual level parameters
  ...
  int_individal_aux    =rnorm(Nsubjects,0, 1);
  slope_individal_aux  =rnorm(Nsubjects,0, 1);
  ...
  int             = pnorm(mu_aux[2]  + sigma_aux[2]*int_individal_aux)*int_range+int_shift;
  slope           = pnorm(mu_aux[3]  + sigma_aux[2]*slope_individal_aux)*slope_range+slope_shift;
  beta            = pnorm(mu_aux[4]  + sigma_aux[3]*beta_indvidial_aux) * beta_range;
  ...
  print(paste(round(mean(alpha),2),round(mean(int),2),round(mean(slope),2),round(mean(beta),2)))
...
source('./models/simulation_Narmed_bandit_task_deltaQ.R')
