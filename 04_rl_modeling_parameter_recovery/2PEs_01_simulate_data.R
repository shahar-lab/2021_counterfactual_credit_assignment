...
#population location parameters
  ...
  beta_range =10
  mu_aux     =c(qnorm(mu_alpha_ch),qnorm(mu_alpha_unch),qnorm(mu_beta/beta_range))
  
  #transform back to natural scale (just for practice)
  mu_alpha_ch   =pnorm(mu_aux[1], 0,1);
  mu_alpha_unch =pnorm(mu_aux[2], 0,1);
  mu_beta       =pnorm(mu_aux[3], 0,1)*beta_range;
  print(paste(round(mu_alpha_ch,2),round(mu_alpha_unch,2),round(mu_beta,2)))
...
#individual level parameters
  ...
  alpha_ch_individal_aux  =rnorm(Nsubjects,0, 1);
  alpha_unch_individal_aux=rnorm(Nsubjects,0, 1);
  beta_indvidial_aux      =rnorm(Nsubjects,0, 1);
  
  #RL parameters per individual given population and group effects
  alpha_ch          = pnorm(mu_aux[1]  + sigma_aux[1]*alpha_ch_individal_aux);
  alpha_unch        = pnorm(mu_aux[2]  + sigma_aux[2]*alpha_unch_individal_aux);
