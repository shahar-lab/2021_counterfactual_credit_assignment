...
model= glmer(stay_frc_unch ~ reward_oneback*delta_abs_exp_value_oneback+(reward_oneback*delta_abs_exp_value_oneback| subject), 
...
df$rt_oneback=lag(df$rt)
model= glmer(stay_frc_unch ~ reward_oneback*rt_oneback+(reward_oneback*rt_oneback| subject), 
             data = df%>%filter(reoffer_ch==F,reoffer_unch==T), 
             family = binomial,
             control = glmerControl(optimizer = "bobyqa"), nAGQ = 0)

plot(effect('reward_oneback:rt_oneback',model))
car::Anova(model)
...
model= lmer(rt ~ reward_oneback*stay_frc_unch+(reward_oneback*stay_frc_unch| subject), 
             data = df%>%filter(reoffer_ch==F,reoffer_unch==T))
car::Anova(model)
plot(effects::effect('reward_oneback:stay_frc_unch',model))
...
m=lm(rt~abs(delta_exp_value),data=df)
library(effects)
plot(effects::effect('delta_exp_value',m))
library(car)
car::Anova(m)
library(lme4)
library(utils)
